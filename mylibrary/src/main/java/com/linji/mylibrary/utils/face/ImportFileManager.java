...
import com.linji.mylibrary.faceHelp.listener.OnImportListener;
import com.linji.mylibrary.faceHelp.manager.FaceSDKManager;
import com.linji.mylibrary.faceHelp.model.ImportFeatureResult;
...
import com.linji.mylibrary.faceHelp.utils.FileUtils;
...
public class ImportFileManager {
    ...
    public void batchImport() {
        ...
        File batchImportDir = FileUtils.getBatchImportSuccessDirectory();
        // 2、遍历该目录下的所有文件
                ...
                try {
                    ...
                    for (int i = 0; i < picFiles.length; i++) {
                        ...
                        if (picName.contains("_")) {
                            name = picName.substring(0, picName.lastIndexOf("_"));
                            picName = name + ".png";
                        } else {
                            name = picName.substring(0, picName.lastIndexOf("."));
                        }
                        Log.e(TAG, "i = " + i + ", picName = " + picName);
                        // 4、判断图片后缀
                        if (!picName.endsWith(".jpg") && !picName.endsWith(".png") &&
                                !picName.endsWith(".PNG") && !picName.endsWith(".JPG") && !picName.endsWith(".svg")) {
                            Log.e(TAG, "图片后缀不满足要求");
...
//                                 更新进度
                        ...
                        if (result.getResult() == 128) {
                            // 将用户信息保存到数据库中
                            /*UserInfo userInfo = LitePal.where("name=?", name).findFirst(UserInfo.class);
                            if (userInfo != null) {
                                boolean importDBSuccess = FaceApi.getInstance().registerUserIntoDBmanager(null,
                                        userInfo.getId() + "", name, name, null, bytes);
                                // 保存数据库成功
                                if (importDBSuccess) {
                                    File facePicDir = FileUtils.getBatchImportSuccessDirectory();
                                    File file = new File(facePicDir, picName);
                                    if (picFile.renameTo(file)) {
                                        success = true;
                                        userInfo.setFacePath(file.getPath());
                                        userInfo.save();
                                    }
                                } else {
                    ...
                    // 导入完成
                    if (mImportListener != null) {
//                        UploadLogUtil.submitLog("同步人脸", "照片数据总数：" + mTotalCount + ", 导入成功：" + mSuccessCount + " ,导入失败：" + mFailCount, SPUtils.getInstance().getString(Constants.USERNAME), "", "");
